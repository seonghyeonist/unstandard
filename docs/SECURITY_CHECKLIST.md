...
4. **Reports** — `POST /api/reports` uses `ReportsRepository` + `ensureReporterProfile` (Supabase alpha adapters). Explicit `REPORTS_PERSISTENCE_ADAPTER=supabase-alpha` required. Fail-closed 503 when disabled. **Still needs migration apply + RLS/staging smoke before alpha.**
