...
2. **Supabase staging login** — server routes only (`/api/auth/supabase/oauth`, magic link server action, `/auth/callback`). Uses `UNSTANDARD_SUPABASE_URL` + `UNSTANDARD_SUPABASE_PUBLISHABLE_KEY` (server-only). **No service role** for user auth. Legacy `NEXT_PUBLIC_SUPABASE_*` is fallback only — do not expand.
...
4. **Reports** — `POST /api/reports` uses `ReportsRepository` + `ensureReporterProfile` (Supabase alpha adapters). Explicit `REPORTS_PERSISTENCE_ADAPTER=supabase-alpha` required. Fail-closed 503 when disabled. Reporter bootstrap nicknames must not be derived from email local-parts. **Still needs migration apply + RLS/staging smoke before alpha.**
