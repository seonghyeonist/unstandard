...
| 게이트 | 현재 | 알파 전 필수 |
|--------|------|--------------|
| 실제 인증 (Supabase Auth) | ⚠️ foundation only; mock dev cookie / no Supabase login UI | ✅ |
| 서버 측 unlock 판정 | ⚠️ signed HttpOnly cookie (not DB) | ✅ DB-backed |
| 프로필 비공개 필드 서버 분리 | ⚠️ server-only module + API (mock) | ✅ Supabase RLS |
| 신고 서버 영속 + 중재 파이프라인 | ⚠️ DB path coded; migration/RLS apply pending | ✅ |
| 차단(block) 기능 | ❌ 미구현 | ✅ |
...
2. **Unlock** — signed HttpOnly cookie (`AUTH_COOKIE_SECRET` 필수 in production). DB 영속 아님. cookie 탈취 시 해당 profile unlock 노출.
3. **Private profile** — `lib/data/mock-private.server.ts` (server-only). 클라이언트 번들에 letter 없음. API는 unlock cookie 검증.
4. **Reports** — `POST /api/reports` → Supabase `reports` via user-scoped RLS client when persistence env is set. **503 if not configured** (no in-memory fallback). `lib/server/report-store.server.ts` is deprecated/unused. `lib/api/report-store.ts` (sessionStorage) is dead client code — not alpha-safe. Migration `0003_reports_dedup_index.sql` dedupes OPEN duplicates.
5. **`middleware.ts`** — Supabase env 있을 때만 세션 검증. Edge 번들에 `@supabase/supabase-js` 경고 — Vercel preview에서 runtime 검증 필요.
6. **`AuthGuard`** — 클라이언트 UX 리다이렉트. 서버 경계는 API + middleware.
...
수동 확인: 약한 답 잠금 유지, 구체적 답 unlock, 신고 API (`POST /api/reports`, Supabase env 필요), UI에 score/Depth Score/path 미노출.
