...
4. **Reports** — `POST /api/reports` uses `ReportsRepository` (Supabase alpha adapter). Fail-closed 503 when adapter env missing. **Still needs migration apply + RLS smoke before alpha.**
