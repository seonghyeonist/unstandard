...
4. **Reports** — `POST /api/reports` uses `ReportsRepository` (Supabase alpha adapter). Requires explicit `REPORTS_PERSISTENCE_ADAPTER=supabase-alpha` **plus** Supabase env; public env alone does not enable. Fail-closed 503 when disabled. **Still needs migration apply + RLS smoke before alpha.**
