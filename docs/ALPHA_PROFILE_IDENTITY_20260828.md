...
- 판정: **IMPLEMENTED_WITH_BLOCKED_IDENTITY_PROVIDER / NOT_READY_FOR_PRODUCTION**
...
**실제 인증사 adapter는 없다.** `getIdentityProvider()`는 항상 null을 반환한다. 실제 실명·전화번호 입력, SMS 발송, 외부 본인인증을 했다는 뜻이 아니다.

- 운영자의 서류 확인, SMS 번호 소유 확인, 통신사 실명 본인확인은 서로 다르다. 아직 창업자의 방식 선택이 없다.
- 현재 계약은 실명 확인과 휴대전화 소유 확인 두 가지를 모두 충족하는 provider를 전제로 한다. SMS만 또는 수동검토를 선택하면 인증 수준·문구·모델을 명시적으로 변경해야 한다.
- 실제 서비스 연결에는 provider 계약/자격, 결제·발송 조건, 서버 자격증명, hosted 입력/복귀 흐름과 서버 결과 검증 adapter, 수탁자·보유기간 고지, 발송 총비용 상한 및 번호별 남용 방지 확인이 필요하다.
...
- 인증을 별도 hosted 입력으로 연결하여 앱에 원문이 들어오지 않는 구조를 전제로 했다. 현재는 provider 미연결이라 아예 수집하지 않는다.
...
1. 인증 수준과 provider 선정 및 계약/개인정보 안내 확정.
2. hosted 인증·서버 검증 adapter 연결. 이름·번호 원문 로그·잔존 검증, 실패/만료/재시도·타인 요청·중복/비용/번호별 abuse 테스트.
