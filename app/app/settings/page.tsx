...
import { endMockSession } from "@/app/login/actions";

export default function SettingsPage() {
  const router = useRouter();
  const queryClient = useQueryClient();
  const mutation = useMutation({
    mutationFn: endMockSession,
    onSuccess: async () => {
  ...
  return (
    <AppShell title="설정" eyebrow="alpha">
      <AuthGuard requireOnboarded={false}>
        <Card>
          <p className="text-sm leading-6 text-foreground/70">알파에서는 필요한 정보만 잠깐 씁니다. 로그아웃하면 서버 세션 쿠키가 삭제됩니다.</p>
