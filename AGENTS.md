...
### Quality gate
- One-shot check: `npm run check` (= `lint` + `typecheck` + `test` + `build`). CI (`.github/workflows/ci.yml`) runs `npm ci`, then `lint`, `typecheck`, `test`, and `build` as explicit steps on Node 20.x/22.x.
